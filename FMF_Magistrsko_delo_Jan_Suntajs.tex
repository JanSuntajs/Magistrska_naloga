...
\begin{equation}
\tau_\mathrm{H}=2\pi\bar{\rho},
\end{equation}
kjer je $\bar{\rho}$ povprečna gostota stanj. Ker tudi izračune $K(\tau)$ izvajamo na razgrnjenem spektru, ima $\bar{\rho}$ v naših računih enotsko vrednost, $\bar{\rho}=1.$ V nadaljevanju bomo čas $\tau$ vselej podajali v enotah $\tau_\mathrm{H},$ kot je to v navadi v literaturi. V teh enotah saturacija nastopi pri $\tau=\tau_\mathrm{H}=1.$\\\\
...
\end{equation}
V tem magistrskem delu privzamemo, da je $J$ konstanten, $J_{i,j}=J.$ Vrzelni in spinski nered lahko uvedemo kot ustrezno linearno kombinacijo, 
\end{appendices}
